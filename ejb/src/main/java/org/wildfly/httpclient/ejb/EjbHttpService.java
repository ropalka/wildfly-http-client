...
public class EjbHttpService {
    ...
    private HttpHandler newInvocationHandler(final RequestType requestType) {
        if (requestType == START_INVOCATION) return new HttpInvocationHandler(association, executorService, localTransactionContext, cancellationFlags, classResolverFilter, httpServiceConfig);
        if (requestType == CANCEL_INVOCATION) return new HttpCancelHandler(association, executorService, localTransactionContext, cancellationFlags);
